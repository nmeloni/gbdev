...
const uint8_t mini_boss_1_move_patterns[][4] = {
    ...
    {BOSS_PATTERN_HEALTH_TRIGGER, 75u, MOVE_PATTERN_LATERAL, SHOOT_PATTERN_AIMED},
    {BOSS_PATTERN_TIME_TRIGGER, 200u, MOVE_PATTERN_SPECIAL_CENTER, SHOOT_PATTERN_NONE},
    {BOSS_PATTERN_HEALTH_TRIGGER, 0u, MOVE_PATTERN_SPECIAL_AIMED, SHOOT_PATTERN_NONE},
    {BOSS_PATTERN_DESTROY, 180u,0,0}
...
inline void handle_boss_pattern(void) {
	...
	if (BOSS.pattern_framecounter > 0) {
	    uint8_t explosion_num = (BOSS.pattern_framecounter >> 3) & 0x03;
...
inline void  handle_boss_move_pattern(void){
	...
	if (BOSS.body.x < PLAYER.body.x) {
	    BOSS.body.dx = 16;
	} else if (BOSS.body.x > PLAYER.body.x) {
	    BOSS.body.dx = -16;
	} else {
	    BOSS.body.dx = 0;
	}
	if (BOSS.body.y < PLAYER.body.y) {
	    BOSS.body.dy = 16;
	} else if (BOSS.body.y > PLAYER.body.y) {
	    BOSS.body.dy = -16;
	} else {
	    BOSS.body.dy = 0;
	}
...
inline void check_boss_hitshots(void) {
	...
	if (check_collision_box(BOSS.body.x, BOSS.body.y,
	    ...
	    if (BOSS.hp <= 0){
		add_explosion(BOSS.body.x, BOSS.body.y);
		BOSS.active = 0;
	    }
